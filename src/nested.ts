...
export function sumPoints(questions: Question[]): number {
    return questions.reduce(
        (total: number, question: Question): number => question.points + total,
        0
    );
}
...
export function editOption(
    ...
    return [
        ...questions.map((question: Question): Question => {
            const result = { ...question, options: [...question.options] };
            if (question.id === targetId) {
                targetOptionIndex !== -1
                    ? result.options.splice(targetOptionIndex, 1, newOption)
...
export function duplicateQuestionInArray(
    ...
    result.splice(index + 1, 0, duplicateQuestion(newId, questions[index]));
    return result;
}
